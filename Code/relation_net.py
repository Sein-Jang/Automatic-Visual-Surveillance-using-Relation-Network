...
def rn_f(g, is_training):
    with tf.device('/gpu:0'):
        ...
        rn_f_4 = tf.layers.dense(inputs=rn_f_3, units=4, name='f_4', activation=None)

    return rn_f_4

def concat_coor(o, i, d):
    coor = tf.tile(tf.expand_dims(
        [float(int(i / d)) / d, (i % d) / d], axis=0), [64, 1])
    o = tf.concat([o, tf.to_float(coor)], axis=1)
    return o

def generate_object(image_object, text_object, is_training):
    #G = tf.reshape(image_object,[64, -1])
    #G = text_object
    #G = tf.concat([tf.reshape(image_object,[64, -1]),text_object], axis=1)
    #"""
